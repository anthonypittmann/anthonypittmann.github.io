...
// Tapping a row to select it (the touch equivalent of hover) is
// deliberately not wired up here -- only on hover-capable (mouse)
// devices. On iPhone it produced a second, independent "selected" row
// alongside the scroll-tracked .in-frame row below, showing two
// divider lines lit at once. The play button's own click handler still
// calls selectRow() regardless of device -- that's an explicit action,
// not an implicit "tap the row" one, and is left alone.
...
// Touch devices have no :hover, so the divider line there (v39 made it
// hover-only on desktop) instead tracks whichever row is currently
// centered in the viewport, continuously updating as the user scrolls.
// This is now the only thing driving the line on touch (see above).
//
// This used to be an IntersectionObserver with a thin rootMargin band,
// but that stopped working reliably on iOS Safari specifically for the
// Spotify row -- a cross-origin iframe inside the observed element
// appears to make isIntersecting unreliable there (a known category of
// WebKit bugs, not something specific to our markup). Plain scroll-
// position math sidesteps that entirely: on every scroll tick, find
// whichever row's own center is closest to the viewport's center and
// mark only that one in-frame. No IntersectionObserver involved.
if (!hasHover) {
  function updateInFrameRow() {
    ...
    tracklistRows.forEach((row) => {
      row.classList.toggle("in-frame", row === closestRow);
    });
  }

  // Same idea as the tracklist rows above, but for the single contact
  // email link rather than picking among several -- just checks
  // whether it's currently sitting within a band around vertical
  // center of the screen.
  const contactEmail = document.querySelector(".contact__email");
  function updateEmailInFrame() {
    ...
    contactEmail.classList.toggle("in-frame", inBand);
  }

  let frameTickScheduled = false;
  window.addEventListener(
    "scroll",
    () => {
      if (frameTickScheduled) return;
      frameTickScheduled = true;
      requestAnimationFrame(() => {
        updateInFrameRow();
        updateEmailInFrame();
        frameTickScheduled = false;
      });
    },
    { passive: true }
  );
...
document.querySelectorAll(".tracklist__player[data-spotify-track]").forEach((player) => {
  // The rounding/clipping lives on this wrapper div, not the iframe
  // itself -- rounding an iframe directly can leave a 1px corner gap
  // (iframes get their own GPU compositing layer, and the rounded clip
  // mask doesn't always cover the highest-curvature point cleanly). A
  // plain div doesn't have that issue, so the iframe sits inside it as
  // a full-bleed rectangle and the wrapper does the actual rounding.
