...
if (!hasHover) {
  const frameObserver = new IntersectionObserver(
    ...
    // Thin band around vertical center of the viewport -- a row only
    // counts as "in frame" while it's crossing roughly the middle 10%
    // of the screen, so only one row (rarely zero, mid-scroll) is ever
    // lit at once.
    { threshold: 0, rootMargin: "-45% 0px -45% 0px" }
  );
  tracklistRows.forEach((row) => frameObserver.observe(row));
}
